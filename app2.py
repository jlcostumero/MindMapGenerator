...
# MapasColeccion.create_index([('delete', 1)], unique=False)


openai.api_key="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"
...
def generar():
    ...
    try:
        ...
        if objeto_json['map'] and objeto_json['map']['name']:
            ...
            return render_template('map.html', texto_generado=texto_generado, idMap=str(uu_id), id=id)
        else:
            return render_template('index.html')
        ...
        nombreFichero = 'ErrorFormat-' + str(uuid.uuid4()) + ".txt"
        with open("diagrams/" + nombreFichero, "w") as file:
...
def diagram(idMap):
    ...
    return render_template('map.html', texto_generado=contenido, nombre=resultado["name"], idMap=idMap)
...
@app.route('/delete/<idMap>', methods=['POST'])
def delete(idMap):
    id =session['user_id']
    filtro = {"idUser": id, "id": idMap}
    nuevos_valores = {'$set': {'delete': True}}

    # Actualizar el registro
    resultados = MapasColeccion.update_one(filtro, nuevos_valores) # borra el registro
    # resultados = MapasColeccion.delete_one(filtro) # borra el registro
    if resultados.deleted_count == 1:
...
def corregir_json(cadena_json):
    ...
    cadena_json_corregida = re.sub(r'\s*\}\s*\}\s*}\s*$', '}}', cadena_json_corregida)
    cadena_json_corregida = re.sub(r'\s*\]\s*\"\s*', '],"', cadena_json_corregida)
...
if __name__ == '__main__':
    ...
    app.run(port=1000,host='0.0.0.0', ssl_context=context)
